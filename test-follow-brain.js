...
console.log(
  `Loaded brain: fitness=${
    brainData.fitness.toFixed(1)
  }, gen=${brainData.generation}, σ=${brainData.optimizer?.sigma?.toFixed(4)}`,
);
console.log(
  `Topology: ${brainData.topology.join("→")}, params: ${brain.paramCount}\n`,
);
...
function runTest(name, target, vel, quat) {
  ...
  for (let frame = 0; frame < MATCH_FRAMES; frame++) {
    ...
    if (frame === 59 || frame === 179 || frame === 359 || frame === 599) {
      ...
      snapshots.push({
        t: ((frame + 1) / 60).toFixed(1),
        dist: dist.toFixed(2),
        speed: speed.toFixed(4),
        angSpeed: angSpeed.toFixed(4),
      });
    }
  }
  ...
  console.log(
    `  Init: dist=${initDist.toFixed(1)}, vel=[${
      vel.map((v) => v.toFixed(3)).join(",")
    }]`,
  );
  console.log(
    `  Final: dist=${finalDist.toFixed(2)}, speed=${
      finalSpeed.toFixed(4)
    }, angSpeed=${finalAngSpeed.toFixed(4)}`,
  );
  console.log(
    `  Min dist reached: ${minDist.toFixed(2)}, fuel used: ${fuelUsed}`,
  );
  console.log(`  Timeline:`);
  for (const s of snapshots) {
    console.log(
      `    t=${s.t}s: dist=${s.dist}, speed=${s.speed}, angSpeed=${s.angSpeed}`,
    );
  }
  console.log();
...
results.push(
  runTest("Off-axis: pure lateral [30,0,0]", [30, 0, 0], [0, 0, 0], [
    0,
    0,
    0,
    1,
...
results.push(
  runTest("Off-axis: above+right [20,20,0]", [20, 20, 0], [0, 0, 0], [
    0,
    0,
    0,
    1,
...
results.push(
  runTest("Lateral velocity [0.3,0,0] + forward target", [0, 0, 30], [
    0.3,
    0,
    0,
  ], [0, 0, 0, 1]),
...
results.push(
  runTest("Close target [3,3,3]", [3, 3, 3], [0, 0, 0], [0, 0, 0, 1]),
);
results.push(
  runTest("Behind with lateral vel", [0, 0, -20], [0.15, 0, 0], [0, 0, 0, 1]),
);

// Tests that previously PASSED (regression check)
console.log("── Regression checks (should still pass) ──\n");
results.push(
  runTest("Forward target [0,0,30]", [0, 0, 30], [0, 0, 0], [0, 0, 0, 1]),
);
results.push(
  runTest("Forward close [0,0,10]", [0, 0, 10], [0, 0, 0], [0, 0, 0, 1]),
);
...
results.push(
  runTest("Pure up + downward vel", [0, 25, 0], [0, -0.1, 0], [0, 0, 0, 1]),
);
results.push(
  runTest("Diagonal far [25,25,25]", [25, 25, 25], [0, 0, 0], [0, 0, 0, 1]),
);
results.push(
  runTest("Behind + fast toward [0,0,-10]", [0, 0, -10], [0, 0, -0.2], [
    0,
    0,
    0,
    1,
...
results.push(
  runTest("Random quat + diagonal far", [25, 25, 25], [0, 0, 0], null),
);
results.push(
  runTest("Random quat + lateral vel", [20, 0, 20], [0.2, 0.1, 0], null),
);
...
if (failed > 0) {
  ...
  for (const r of results.filter((r) => !r.pass && r.finalDist >= 5)) {
    console.log(
      `    - ${r.name}: dist=${r.finalDist.toFixed(2)}, speed=${
        r.finalSpeed.toFixed(4)
      }`,
    );
  }
}
