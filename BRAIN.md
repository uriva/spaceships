...
- **Hit logic**: When fire output > 0 and battery ≥ WEAPON_COST, raycast
  forward. Hit the closest enemy within the 5° cone and range.
...
- **Parameters**: ~(43×32+32) + (32×32+32) + (32×5+5) = 1408 + 1056 + 165 =
  **2629 params**.
...
- **Proximity reward**: each frame, accumulate `1 / (1 + dist)`. Closer = more
  points. Summed over all 3000 frames, so sustained proximity matters more than
  a brief flyby.
- **Fuel efficiency bonus**: `(fuel_remaining / fuel_initial) × F` where F is a
  tuning weight. Rewards finishing with fuel left. A ship that tracks the target
  AND conserves fuel scores higher than one that burns everything.
...
- **σ₀**: 0.5.
- **Episodes per eval**: 12 (4 short-range + 4 long-range + 4 low-fuel). All
  have asteroids and moving targets.
